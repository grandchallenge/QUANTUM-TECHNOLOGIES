...
EXPECTED_PROTECTED_BASE="4182946ec5bdfda9de1443124624bd084a5acfda"
COLAB_AUTH="${COLAB_AUTH:-oauth2}"
VENV_PY="$ROOT/.venv/bin/python"
COLAB_BIN="$ROOT/.venv/bin/colab"
JOB_TEMPLATE="$ROOT/configs/compute/qtr_c90_structure_001_job.json"
...
[[ -f "$JOB_TEMPLATE" ]] || { echo "missing structure job template" >&2; exit 2; }

HEAD="$(git rev-parse HEAD)"
test -z "$(git status --porcelain)" || { echo "ABORT: working tree not clean" >&2; exit 20; }
git merge-base --is-ancestor "$EXPECTED_PROTECTED_BASE" "$HEAD" || {
  echo "ABORT: protected structural predecessor is not an ancestor of HEAD" >&2
  exit 21
}

RUN_ID="$(date -u +%Y%m%dT%H%M%SZ)-$$"
SESSION="gcl-qtr-c90-structure-${RUN_ID,,}"
LOCAL_RUN="$ROOT/runs/hosted/QTR-C90-STRUCTURE-001/$RUN_ID"
mkdir -p "$LOCAL_RUN"
...
echo "[QTR-C90-STRUCTURE] source=$HEAD"
echo "[QTR-C90-STRUCTURE] protected predecessor=$EXPECTED_PROTECTED_BASE"
echo "[QTR-C90-STRUCTURE] allocating fresh CPU session=$SESSION"
...
set +e
"$COLAB_BIN" --auth="$COLAB_AUTH" exec -s "$SESSION" -f "$ROOT/colab/qtr_c90_structure_remote_job.py" --timeout "$REMOTE_TIMEOUT" > >(tee "$LOCAL_RUN/remote-stdout.txt") 2> >(tee "$LOCAL_RUN/remote-stderr.txt" >&2)
REMOTE_RC=$?
set -e
...
for required in experiment_receipt.json gcl_output_bundle.tar.gz gcl_result.json; do
  [[ -f "$LOCAL_RUN/$required" ]] || { echo "ABORT: missing hosted artifact $required; evidence retained at $LOCAL_RUN" >&2; exit 30; }
done
...
if r.get("hosted_session_identity")!=session or result.get("hosted_session_identity")!=session: raise SystemExit("session identity mismatch")
...
if result.get("manifest_payload_sha256")!="205ecca612ae366694d4c17b6ce518727abf80114b6e52598073b238946f2a6a":
    raise SystemExit("structure manifest identity drift")
print("[QTR-C90-STRUCTURE] Phase C:",result["phase_b_c"]["phase_c_disposition"])
print("[QTR-C90-STRUCTURE] eligible:",result["phase_b_c"]["eligible_methods"])
print("[QTR-C90-STRUCTURE] Phase D attempted:",result["phase_d"]["attempted"])
print("[QTR-C90-STRUCTURE] Phase E attempted:",result["phase_e"]["attempted"])
PY

if [[ "$REMOTE_RC" -ne 0 ]]; then
  echo "ABORT: remote process rc=$REMOTE_RC; evidence retained at $LOCAL_RUN" >&2
  exit "$REMOTE_RC"
fi

echo "RESULT: QTR_C90_STRUCTURE_001_HOSTED_RUN_COMPLETE"
