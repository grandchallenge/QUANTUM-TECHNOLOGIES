...
EXPECTED_PROTECTED_BASE="8b52c71c916e9eea4a4c76309846cdb2b4a7d55a"
COLAB_AUTH="${COLAB_AUTH:-oauth2}"
VENV_PY="$ROOT/.venv/bin/python"
COLAB_BIN="$ROOT/.venv/bin/colab"
JOB_TEMPLATE="$ROOT/configs/compute/qtr_c90_exact_requal_001_job.json"
...
[[ -f "$JOB_TEMPLATE" ]] || { echo "missing C90 job template" >&2; exit 2; }

HEAD="$(git rev-parse HEAD)"
test -z "$(git status --porcelain)" || { echo "ABORT: working tree not clean" >&2; exit 20; }

RUN_ID="$(date -u +%Y%m%dT%H%M%SZ)-$$"
SESSION="gcl-qtr-c90-${RUN_ID,,}"
LOCAL_RUN="$ROOT/runs/hosted/QTR-C90-EXACT-REQUAL-001/$RUN_ID"
mkdir -p "$LOCAL_RUN"
...
echo "[QTR-C90] source=$HEAD"
echo "[QTR-C90] protected predecessor=$EXPECTED_PROTECTED_BASE"
echo "[QTR-C90] allocating fresh CPU session=$SESSION"
...
set +e
"$COLAB_BIN" --auth="$COLAB_AUTH" exec -s "$SESSION" -f "$ROOT/colab/qtr_c90_exact_remote_job.py" --timeout "$REMOTE_TIMEOUT" > >(tee "$LOCAL_RUN/remote-stdout.txt") 2> >(tee "$LOCAL_RUN/remote-stderr.txt" >&2)
REMOTE_RC=$?
set -e
...
print("[QTR-C90] phase M:",result["phase_m"]["adjudication_status"])
print("[QTR-C90] phase X attempted:",result["phase_x"]["attempted"])
print("[QTR-C90] validation attempted:",result["validation"]["attempted"])
PY
...
echo "RESULT: QTR_C90_EXACT_REQUAL_HOSTED_RUN_COMPLETE"
