...
class QTRC90ExactSymbolic001Tests(unittest.TestCase):
    SCOPES = [(0,), (0, 1), (1,)]
    SELECTOR_QUBITS = [0, 2]
    ORDER = [0, 1]
    ...
    def _brute(self, algebra: str, selector_coordinate: int):
        selector_parameter = {q: i for i, q in enumerate(self.SELECTOR_QUBITS)}
        assignments = []
        for assignment in range(1 << len(self.ORDER)):
            ...
            for qubit, scope in enumerate(self.SCOPES):
                bit = (
                    (selector_coordinate >> selector_parameter[qubit]) & 1
                    if qubit in selector_parameter
                    else 0
                )
                for variable in scope:
                    bit ^= stabilizer_bits[variable]
    ...
    def test_symbolic_matches_dd_and_bruteforce_for_every_selector(self) -> None:
        for algebra in DAG.ALGEBRAS:
            old_dag, old_root = self._compile_dd(algebra)
            new = Symbolic.compile_factor_graph(
                scopes=self.SCOPES,
    ...
    def test_symbolic_compile_is_deterministic(self) -> None:
        for algebra in DAG.ALGEBRAS:
            left = Symbolic.compile_factor_graph(
                scopes=self.SCOPES,
                selector_qubits=self.SELECTOR_QUBITS,
                order=self.ORDER,
                algebra=algebra,
            )
            right = Symbolic.compile_factor_graph(
                scopes=self.SCOPES,
                selector_qubits=self.SELECTOR_QUBITS,
                order=self.ORDER,
                algebra=algebra,
