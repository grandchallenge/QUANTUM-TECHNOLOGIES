...
import qtr_c90_exact_jointdd_001 as Joint


class QTRC90ExactJointDD001Tests(unittest.TestCase):
    SCOPES = [(0,), (0, 1), (1,)]
    SELECTOR_QUBITS = [0, 2]
    ORDER = [0, 1]
        ...
        factors = [
            (
                ...
                DAG._local_symbolic_factor(
                    qubit, scope, selector_parameter, algebra, dag, dd
                ),
            )
    ...
    def test_joint_mtbdd_matches_old_dd_and_bruteforce(self) -> None:
        for algebra in DAG.ALGEBRAS:
            old_dag, old_root = self._compile_dd(algebra)
            new = Joint.compile_factor_graph(
                scopes=self.SCOPES,
                selector_qubits=self.SELECTOR_QUBITS,
                order=self.ORDER,
                algebra=algebra,
            )
            self.assertTrue(all(node[0] in {"T", "I"} for node in new["dag"].nodes))
    ...
    def test_joint_mtbdd_is_deterministic(self) -> None:
        for algebra in DAG.ALGEBRAS:
            left = Joint.compile_factor_graph(
                scopes=self.SCOPES,
                selector_qubits=self.SELECTOR_QUBITS,
                order=self.ORDER,
                algebra=algebra,
            )
            right = Joint.compile_factor_graph(
                scopes=self.SCOPES,
                selector_qubits=self.SELECTOR_QUBITS,
                order=self.ORDER,
                algebra=algebra,
