...
def loadj(p,sha):
    d=json.loads(p.read_text());assert d.get("payload_sha256")==sha and canon(d)==sha;return d
def manifest():
    ...
    return d
def preds():
    r=loadj(ROOT/"evidence/QTR-C90-RESOURCE-ENVELOPE-001-report.json",RP)
    s=loadj(ROOT/"evidence/QTR-C90-STRUCTURE-001-report.json",SP)
    c=loadj(ROOT/"evidence/QLDPC-SCALE-001A-report.json",CP)
